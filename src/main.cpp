...
static EthernetClient base_client;
static SSLClient bearSSLClient(base_client, TAs, (size_t)TAs_NUM, rand_pin);
//static BearSSLClient bearSSLClient(base_client);
...
void connectToEthernet()
{
  Logger.Info("Attempting to connect to Ethernet");
  ...
  while (Ethernet.linkStatus() != EthernetLinkStatus::LinkON)
  {
    Serial.print(".");
    delay(IOT_CONFIG_ETHERNET_CONNECT_RETRY_MS);
  }
  Serial.println();
  ...
  while (true)
  {
    unsigned long time123 = getTime();
    Serial.print("getTime(): ");
    Serial.println(time123);
